...
(전 마일스톤 M1–M6 완료 — 잔여 작업은 사용자 시각 검수 피드백 기반 개선)

## NEXT (위에서부터 순서대로 NOW로 승격)

> 공통 규칙: **각 마일스톤 M{n}의 마지막 태스크에는 CLAUDE.md 'Git/배포 규약'의 dev→main 머지(=자동 배포)와 `gh run watch` 성공 확인까지 포함**된다.

(없음 — 시각 검수에서 발견되는 이슈를 태스크로 추가할 것)

## DONE

- [x] **M6-3** 성능 패스 (M6 마감): lint 0 에러(ref 캐시 → 렌더 중 상태 조정 패턴 수정), 모바일 quality 프리셋(터치 기기 파장샘플 4), 청크 한도 명시. 60fps 실측은 사용자 검수 대기. release: M6 배포 (2026-06-11)
...
- 마지막 작업 파일/함수: M6-3 정리 — CivicClock/EventCalendar(렌더 중 상태 조정 패턴), SkyDome(모바일 프리셋), vite.config
- 어디까지 했나: **전 마일스톤 M1–M6 완료, release: M6 배포** — System View·시간계·시민시계·Surface View(항성/별/행성/대기/플레어/지형)·이벤트 캘린더·설정 패널 전부 라이브
- 다음 세션 첫 행동: 사용자 시각 검수 결과 청취 → 발견 이슈를 NOW 태스크로 등록. 검수 포인트: ①터미네이터 박명 그라데이션(§6.5) ②c 충 원반 질감/글레어(§7.5) ③칭동 일출 연출 ④60fps@1080p
- 주의사항/함정: 시각 품질 상수(셰이더 강도·노출·블룸)는 모두 컴포넌트/셰이더 안에 있음 — 물리 상수와 달리 자유 조정 가능. 엄폐 지속시간은 스펙 ~1.4h가 아닌 실계산 ~2.2h가 맞음(DECISIONS 참조)
- 테스트 상태: 73 passed / 0 failed (전체 14개 파일 — §10 테스트 8종 전부 구현·통과)
